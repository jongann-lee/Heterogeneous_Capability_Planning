"""Immutable device-resident representation of the fixed WV world."""
...
class TensorWorld:
    ...
    def from_networkx(cls, graph, candidate_config, device="cuda"):
        ...
        visible_targets = torch.zeros(
            (len(nodes), len(targets)), dtype=torch.bool, device=device)
        for node in nodes:
            visible = {node}
            ...
            if explicit is not None:
                ...
                for u, v in graph.nodes[node].get("visible_edges", []):
                    visible.update((u, v))
            for target in visible & set(targets):
                visible_targets[node_index[node], target_index[target]] = True

        max_degree = max(dict(graph.out_degree()).values())
        edge_cost = torch.full(
            (len(nodes), max_degree), torch.inf, dtype=torch.float32,
            device=device)
        # Slots align with a separately materialized neighbor tensor below.
        neighbor_slots = {node: 0 for node in nodes}
        neighbors = torch.full(
            (len(nodes), max_degree), -1, dtype=torch.long, device=device)
        for u, v, weight in graph.edges(data="distance"):
            row, slot = node_index[u], neighbor_slots[u]
        ...
        target_candidate_mask = (
            candidate_nodes[:, None] == torch.tensor(
                [node_index[node] for node in targets], device=device)[None, :])
        distance_scale = max(sum(float(data.get("distance", 1.0))
                                 for _u, _v, data in graph.edges(data=True)), 1.0)
        target_sssp = router.sssp([node_index[node] for node in targets], 0)
        ...
        world = cls(
            graph, nodes, node_index, targets, candidates, positions, heights,
            torch.tensor([node_index[node] for node in targets],
